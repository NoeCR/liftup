...
void main() {
  group('Per-routine progression', () {
    ...
    test('la progresión se calcula por rutina (llamadas incluyen routineId)', () async {
      ...
      when(
        mockService.calculateProgression(configId, exerciseId, routineX, 100.0, 5, 3, exercise: anyNamed('exercise')),
      ).thenAnswer(
        (_) async =>
            ProgressionCalculationResult(newWeight: 102.5, newReps: 5, newSets: 3, incrementApplied: true, reason: 'X'),
      );
      when(
        mockService.calculateProgression(configId, exerciseId, routineY, 80.0, 8, 3, exercise: anyNamed('exercise')),
      ).thenAnswer(
        (_) async =>
      ...
      final x = await mockService.calculateProgression(configId, exerciseId, routineX, 100.0, 5, 3, exercise: exercise);
      final y = await mockService.calculateProgression(configId, exerciseId, routineY, 80.0, 8, 3, exercise: exercise);
