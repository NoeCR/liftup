...
void main() {
  group('DoubleFactorProgressionStrategy (Doble Progresión)', () {
    ...
    test('incrementa peso en semana impar (semana 1)', () {
      ...
      expect(result.newWeight, 73.75); // Peso incrementa en 3.75kg (semana impar, barbell multiJoint, nivel intermedio)
    ...
    test('usa parámetros por ejercicio cuando están disponibles', () {
      ...
      expect(result.newWeight, 73.75); // increment_value específico (3.75kg)
      expect(
        result.newReps,
        8,
    ...
    test('aplica deload correctamente', () {
      ...
      expect(result.newSets, 4); // Sets reducidos al 70% (3 * 0.7 = 2.1 ≈ 2)
    ...
    test('usa valores por defecto cuando no hay parámetros personalizados', () {
      ...
      expect(result.newWeight, 73.75); // incrementValue adaptativo (3.75kg para barbell multiJoint, nivel intermedio)
      expect(result.newReps, 5); // reps se mantienen en minReps (semana impar)
    });
    ...
    test('simula 3 ciclos completos con deload cada 6 semanas para validar progresión a largo plazo', () {
      ...
      for (var entry in progressionHistory) {
        ...
        if (entry['isDeload'] as bool) {
          expect(sets, lessThanOrEqualTo(4)); // Sets reducidos durante deload
        } else {
